    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px' }}>
      ...
      <div style={{ padding: '10px 0' }}>
        ...
        <p style={{ color: 'var(--text-muted)', fontSize: '16px', margin: 0 }}>
          {user?.role === 'mechanic'
            ? 'Manage your customer appointments, track repairs, and process payments.'
            : user 
      ...
      {/* Main Grid */}
      <div className={user?.role === 'mechanic' ? "mechanic-dashboard-grid" : "grid-3"} style={user?.role === 'mechanic' ? { display: 'flex', flexDirection: 'column', gap: '20px' } : {}}>
        {/* Column 1: Garage Manager (Hidden for Mechanics) */}
        {user?.role !== 'mechanic' && (
        ...
        {/* Column 2: Quick Booking Shortcuts (Hidden for Mechanics) */}
        {user?.role !== 'mechanic' && (
          <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', minHeight: '380px' }}>
            <h3>✨ Services</h3>
        ...
        <div className="glass-card" style={{ display: 'flex', flexDirection: 'column', minHeight: '380px' }}>
          <h3>{user?.role === 'mechanic' ? '📅 My Jobs & Payments' : '📅 Appointment Queue'}</h3>
            ...
            {user ? (
              bookings.length > 0 ? (
                  ...
                  <div key={b._id || b.id} style={{ background: 'rgba(255,255,255,0.02)', padding: '14px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                      <div style={{ fontWeight: '600', fontSize: '14px' }}>
                        {user?.role === 'mechanic' ? `Customer: ${b.user_id}` : b.center_name}
                      </div>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px' }}>
                      ...
                      >
                        {user?.role === 'mechanic' ? 'Update Status →' : 'Track Service →'}
                      </button>
                    </div>
                  </div>
                ))
                ...
                <div style={{ textAlign: 'center', padding: '60px 0', color: 'var(--text-muted)', fontSize: '14px' }}>
                  {user?.role === 'mechanic' ? 'No incoming jobs right now.' : 'No service appointments scheduled. Select a service to get started!'}
                </div>
              )
            ) : (
